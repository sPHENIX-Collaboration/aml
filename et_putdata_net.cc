...
et_putdata_net   -n <number of buffers> -w <wait in secs> -H <approx hertz>  filename
...
void exitmsg()
{
  std::cout << "** usage: et_putdata_net  -n numberofevents -w waitinseconds  -H hz datafile etname" << std::endl;
  exit(0);
}
...
int main( int argc, char* argv[])
{
  ...
  */
  ...
  if (et_open(&id, et_name, openconfig) != ET_OK)
    {
      printf("%s: et_open problems for %s\n", argv[0], et_name);
      exit(1);
    }
