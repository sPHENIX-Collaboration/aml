...
void exitmsg()
{
  cout << "** This is the et_server (threaded version). No gimmicks. Pure Power." << endl;
  cout << "** usage: et_server_nothread  [-t token] et_name port-number" << endl;
  exit(0);
}
  ...
  while (  go_on  )
	...
	case  CTRL_DATA:
	  ...
	  
	  {
		...
		cout << "<E> et_server: Bad length, buffer contained " << n << " too many bytes, flushed them, token is " << token <<  endl; 
		i = CTRL_REMOTEFAIL;
	      }
	  }
...
void * disassemble_buffer(void *arg)
{
  ...
  len = (unsigned long long ) intarg[3];
  //  cout << "in disassemble_buffer, (buffer) len = " << len << endl;
  buffer *bptr = new buffer( bp, len );
